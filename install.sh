...
echo -e "+----------------------------------------------------+" ${Color_Off}
sudo http_proxy=$http_proxy apt install -y git curl wget gparted apt-transport-https build-essential vlc htop easystroke intltool unrar-free unzip gdebi net-tools vim
...
echo -e ${RED}"\n BlueJeans" ${Color_Off}
wget -e use_proxy=yes -e http_proxy=$http_proxy -O bluejeans.deb "http://172.16.0.12/software/BlueJeans_2.25.0.78.deb"
...
wget -e use_proxy=yes -e http_proxy=$http_proxy -O iclient.deb "http://172.16.0.12/software/icaclient_21.12.0.18_amd64.deb"
sudo dpkg -i ./iclient.deb


rm -rf ./iclient.deb ./bluejeans.deb ./microsoft.gpg
